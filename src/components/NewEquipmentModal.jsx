// src/components/NewEquipmentModal.jsx
import React, { useState, useContext } from 'react';
import { X } from 'lucide-react';
import { AppContext } from '../context/AppContext';
import { saveEquipment } from '../services/firebaseService';

const NewEquipmentModal = ({ onClose }) => {
 const { divisions, equipment, setEquipment } = useContext(AppContext);
 ...
 const generateEquipmentId = () => {
   // Get abbreviation from equipment name (first 4 chars) or division
   let prefix = 'EQUIP';
   if (formData.name) {
   ...
   const existingIds = equipment
     ...
     .map(item => parseInt(item.id.split('-')[2] || '0'));
 ...
 const handleChange = (e) => {
   ...
   // Auto-fill parts info if not already filled
   if (name === 'make' && value && !formData.partsMake) {
     setFormData(prev => ({
       ...prev,
       partsMake: value
     }));
   ...
   if (name === 'model' && value && !formData.partsModel) {
     setFormData(prev => ({
       ...prev,
       partsModel: `${value}-PARTS`
     }));
   }
 };
   ...
   if (validateForm()) {
     ...
     try {
       ...
       const newEquipment = {
         ...
         partsVin: formData.partsVin.trim() || `PARTS-${formData.make.substring(0, 3)}-${equipmentId}`
       };
       
       // Save to Firebase
       const savedEquipment = await saveEquipment(newEquipment);
       
       // Update equipment list with the saved item (including Firebase document ID)
       setEquipment(prevEquipment => [savedEquipment, ...prevEquipment]);
       
       // Close form
       onClose();
     } catch (error) {
       console.error("Error saving equipment:", error);
       alert("There was an error saving the equipment. Please try again.");
